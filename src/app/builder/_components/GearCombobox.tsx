      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button
            ...
            role="combobox"
            aria-expanded={open}
            className="w-[70px] h-[70px] justify-center relative rounded"
          >
            {selectedItem ? (
              <HoverCard>
                <HoverCardTrigger asChild>
                  <img src={selectedImage} alt={selectedGear} className="h-full w-full object-cover rounded" />
                </HoverCardTrigger>
                <HoverCardContent className="w-[300px] p-4">
                  <div className="flex items-center space-x-2">
                    <img src={selectedItem.imageSrc} alt={selectedItem.label} className="w-8 h-8" />
                    <div>
                  ...
                  <div className="mt-2">
                    <p className={cn("text-sm", selectedGear === 'Weapon' ? "text-foreground" : "text-muted-foreground")}>
                      Weapon: {gemStats[selectedItem.value]?.stats.weapon}
                    </p>
                    <p className={cn("text-sm", selectedGear === 'Armor' ? "text-foreground" : "text-muted-foreground")}>
                      Armor: {gemStats[selectedItem.value]?.stats.armor}
                    </p>
                    <p className={cn("text-sm", isJewelrySlot(selectedGear) ? "text-foreground" : "text-muted-foreground")}>
                  ...
                  <HoverCard key={item.value}>
                    <HoverCardTrigger asChild>
                      ...
                      >
                        ...
                        />
                        <img src={item.imageSrc} alt={item.label} className="w-4 h-4 mr-2" />
                        {item.label}
                      </CommandItem>
                    </HoverCardTrigger>
                    <HoverCardContent className="w-[300px] p-4">
                      <div className="flex items-center space-x-2">
                        <img src={item.imageSrc} alt={item.label} className="w-8 h-8" />
                        <div>
